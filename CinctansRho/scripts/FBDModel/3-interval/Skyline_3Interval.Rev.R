...
#     timeline <- v(.5, 4.5, 9)
      timeline <- v(0.9, 2.3, 3.9, 6.3, 8.4, 10.8);
     ...
     fbd_dist = dnBDSTP(originAge=origin_time, lambda=speciation_rate, mu=extinction_rate, psi=psi, rho=rho, timeline=timeline, taxa=taxa, condition="sampling")
 ...
 moves.append( mvFossilTimeSlideUniform(fbd_tree, origin_time, weight=5.0) )
     ...
     n_max_states <- 3
     idx = 1
     morpho_f_bystate <- morpho_f.getNumStatesVector()
     ...
     n_max_states <- 4
     morpho_nf_bystate <- morpho_nf.getNumStatesVector()
     for (i in 1:n_max_states) {
